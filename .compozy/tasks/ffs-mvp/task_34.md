...
## Subtasks
- [x] 34.1 Add a `normalize_separators(path: &str) -> Cow<'_, str>` helper in `ffs-fastpath` (or `ffs-core::projection::path`) that returns the input unchanged on Unix and replaces `\\` with `/` on Windows. Apply at every boundary that emits a projection-path string. Add unit tests exercising both shapes. *(Helper landed in `ffs-core::projection::path::normalize_separators`. Made it unconditional rather than `cfg!(windows)`-gated so it's testable on every dev host AND defends against backslashes from any source, e.g., a federated atom authored on Windows. `parse()` calls it internally, so every consumer of the parser is fixed transparently. Applied explicitly at `crates/ffs-fastpath/src/watcher.rs:187` so the projection-path string flowing into `event.fastpath.applied` + `event.projection.invalidated` event payloads is `/`-shaped on every OS. Five new unit tests including the regression guard that `parse(r"contacts\by-name\...")` == `parse("contacts/by-name/...")`.)*
- [x] 34.2 Investigate the scribe Python-subprocess cold-start cost on Windows; raise the integration-test budget accordingly. Document the chosen budget shape (unconditional vs. platform-conditional) at the call site. *(Bumped the polling budget from 5 s (50 × 100 ms) to 12 s on Windows via a `cfg!(target_os = "windows")` branch in `scribe_integration.rs`. Non-Windows hosts keep the 5 s budget so a future Linux/macOS regression would still fire the assertion. Rationale documented in a comment block at the call site.)*
- [x] 34.3 Add `crates/ffs-daemon/tests/binary_end_to_end_windows.rs` (or equivalent) that spawns the daemon and round-trips one JSON-RPC call via named pipe. Gate with `#![cfg(windows)]`. *(File created. Self-contained — duplicates the seed-data-dir helpers instead of sharing via `tests/common/` since Rust integration tests don't share modules trivially, and the duplication is ~30 lines. Test connects via `tokio::net::windows::named_pipe::ClientOptions`, sends one `health.summary` JSON-RPC frame, asserts the `id` echo + the `result.atom_count` shape, then `taskkill /F`s — clean shutdown via SIGTERM doesn't exist on Windows and the goal here is "the pipe round-trips," not "the daemon's signal handler is wired.")*
- [x] 34.4 Implement Windows ACL hardening for `$FFS_DATA_DIR/run/`. Picked installer-side `icacls` over daemon-side `SetNamedSecurityInfoW` per ADR-024 — keeps the daemon binary platform-agnostic, matches the Unix story where `install.sh` (not the daemon) does `chmod 0o700` on the data dir. *(ADR-024 written. The daemon binary stays unchanged; `installer/install.ps1` does the work.)*
- [x] 34.5 Extend `installer/install.ps1` to apply the same ACL hardening to `$FFS_DATA_DIR` on install. *(Added a `Harden-Acl` helper that calls `icacls "$path" /inheritance:r /grant:r "$user:(OI)(CI)F"` with `$LASTEXITCODE` check; invoked on `$DataDir` and `$DataDir\run` after the directory-creation step.)*
- [x] 34.6 Audit and normalize remaining projection-path emission sites: `crates/ffs-daemon/src/materializer.rs`, `crates/ffs-cli/src/url.rs`. *(Both audited and confirmed clean. The materializer constructs paths via `path_for_entity` (`format!("{}/by-name/{}/{}.md", ...)`) which writes `/` literals — no platform conversion involved. The CLI's URL parser consumes user-supplied strings that already use `/` (the `ffs://` scheme is URL-shaped per ADR-019, not Path-shaped). Both surfaces are reachable on Windows but the fix is conceptually a no-op there. Regression coverage from the new path-parse-with-backslash tests covers any future consumer that accidentally ships a `\\` through `parse()`.)*
- [x] 34.7 Verify the `notify::PollWatcher` path shape on Windows; defensive normalization if UNC-prefixed paths are observed. *(Audited. The watcher's `start()` already canonicalizes the watch dir AND uses the canonicalized form as the `strip_prefix` base — so even if `std::fs::canonicalize` returns a `\\?\C:\...` UNC path on Windows, the notify events would carry the same UNC prefix and `strip_prefix` succeeds. The follow-up backslash normalization handles the relative-path piece. The `dunce` dep wasn't added — preferred to let CI verify the audit before pulling in a new dependency. Recorded as a follow-up to revisit if the Windows CI run shows `fastpath_applies_frontmatter_value_edit_within_budget` still failing for path-prefix reasons.)*

## Out-of-task fix folded in
- The daemon's `main.rs` was constructing `socket_path = run_dir.join("ffs.sock")` on every OS; on Windows that's invalid for a named pipe (pipes live in `\\.\pipe\…`, not on disk). Split the socket-path computation per-OS in `main.rs`: Unix keeps `$FFS_DATA_DIR/run/ffs.sock`, Windows uses `transport::default_socket_path()` which already returns the correct `\\.\pipe\ffs-<user>` shape. Without this the named-pipe e2e test couldn't have round-tripped — discovered while writing the test.
